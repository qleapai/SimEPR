...
def easyspin_script(fit: FitResult, mw_frequency_GHz: float = 9.85) -> str:
    lines = [
        "% Auto-generated by SimEPR",
        "% Review all N-associated assignments against controls before manuscript use.",
        f"Exp.mwFreq = {mw_frequency_GHz:.6g};",
        f"Exp.Range = [{fit.field_mT.min():.6g} {fit.field_mT.max():.6g}];",
        "",
    ]
    for i, component in enumerate(fit.components, 1):
        nucs = ",".join(n.isotope for n in component.nuclei)
        a_vals = " ".join(f"{n.A_mT:.6g}" for n in component.nuclei)
        lines.extend(
            [
                f"Sys{i}.g = {component.g:.7g};",
                f"Sys{i}.lwpp = {component.linewidth_mT:.6g};",
                f"Sys{i}.weight = {fit.weights.get(component.component_id, component.weight):.6g};",
            ]
        )
        if n_vals := a_vals:
            lines.append(f"Sys{i}.Nucs = '{nucs}';")
            lines.append(f"Sys{i}.A = [{n_vals}];")
        lines.append("")
    lines.append("% Use garlic(Sys,Exp) or pepper(Sys,Exp) depending on your EasySpin workflow.")
    return "\n".join(lines)
